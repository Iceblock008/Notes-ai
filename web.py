# ─── FILE: web.py ────────────────────────────────────────────────────────────
# Mobile-friendly web server for the Video Knowledge Agent
# Run:  python web.py
# Then open http://<your-local-ip>:8081 on your phone
...
async function process() {
  ...
  document.getElementById('goBtn').textContent = '...';    setStep('s1', 'active');
...
async def api_process(request: Request):
    ...
    try:
        ...
        # If it starts with [ERROR] it's an error message from run_agent
        if result.startswith("[ERROR]"):
            return {"status": "error", "error": result[7:]}

        # Parse type and title from the result (first two lines)
...
def get_local_ip():
    ...
    try:
        ...
        s.settimeout(0)
        # 8.8.8.8 is just a target; no data is sent
